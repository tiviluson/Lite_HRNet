...
class SpatialWeighting(nn.Module):
    ...
    def __init__(self,
        ...
        # self.global_avgpool = F.nn.AvgPool2d(1)
    ...
    def forward(self, x):
        kernel_size = [int(s) for s in x.size()[-2:]]
        out = F.avg_pool2d(x, kernel_size=kernel_size)
        out = self.conv1(out)
        out = self.conv2(out)
        return x * out
...
class CrossResolutionWeighting(nn.Module):
    ...
    def forward(self, x):
        ...
        for s in x[:-1]:
            ...
            stride = (int((ssize[0]-kernel[0])/(mini_size[0]-1)),
                      int((ssize[1]-kernel[1])/(mini_size[1]-1)))
